...
class Usuario_agencia(models.Model):
    ...
    correo=models.EmailField(max_length=100,help_text="Ingerse su Correo Electronico")
    password=models.CharField(max_length=100,help_text="Ingerse Password")
    def __str__(self):
        return self.nombre
...
class Evento_agencia(models.Model):
    ...
    distrito=models.ForeignKey(Distrito_agencia(), on_delete=models.CASCADE)
    distrito=models.ForeignKey(Galeria_agencia(), on_delete=models.CASCADE)
    def __str__(self):
        return self.nombre
